...
# Define FaceVerificationResult locally to avoid dependency on schemas.py
# This keeps the module self-contained as designed
class FaceVerificationResult:
    def __init__(self, match: bool, distance: float, match_score: float, model_used: str,
                 is_real: Optional[bool] = None, antispoofing_score: Optional[float] = None):
        self.match = match
        self.distance = distance
        self.match_score = match_score
        self.model_used = model_used
        self.is_real = is_real
        self.antispoofing_score = antispoofing_score
    ...
    def model_dump(self):
        return {
            ...
            "antispoofing_score": self.antispoofing_score,
        }
...
def verify_face(
    ...
    try:
        ...
        result = _deepface_verify(document_image_path, selfie_image_path, enforce_detection=True)
        ...
        logger.warning(f"Face verification strict mode failed: {e} — retrying with relaxed detection on ID photo")
        try:
            from deepface import DeepFace
            result = _deepface_verify(document_image_path, selfie_image_path, enforce_detection=False)
            distance = float(result["distance"])
            threshold = float(result["threshold"])
            match = bool(result["verified"])
            match_score = _distance_to_score(distance, threshold)
            ...
            logger.warning(f"Face verification failed - no face detected: {retry_exc}")
            return FaceVerificationResult(
...
def _deepface_verify(document_image_path, selfie_image_path, enforce_detection: bool):
    from deepface import DeepFace
