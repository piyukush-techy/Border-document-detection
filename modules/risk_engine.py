...
FACE_MATCH_HARD_GATE_THRESHOLD = 18.0          # face_similarity, 0-100 (ID photos are low quality)
BLINK_LIVENESS_HARD_GATE_THRESHOLD = 20.0      # single-frame liveness is noisy — only gate obvious spoofs
...
def _face_soft(inp: RiskEngineInput) -> float:
    """0-1 confidence. Uses neutral score when face detection failed or Aadhaar photo is too small."""
    if (
        inp.face_similarity <= 1.0
        and inp.face_distance is not None
        and inp.face_distance >= 0.99
    ):
        return 0.5
    if inp.document_type in ("national_id", "aadhaar") and inp.face_similarity < FACE_MATCH_HARD_GATE_THRESHOLD:
        return 0.55  # inconclusive ID photo — don't drag the score to FAKE

    span = max(1.0, 100.0 - FACE_MATCH_HARD_GATE_THRESHOLD)
...
def _liveness_soft(inp: RiskEngineInput, reasons: List[str]) -> float:
    ...
    if inp.document_type in ("national_id", "aadhaar") and blink_conf < 0.5:
        blink_conf = 0.55  # still-photo selfie of a live officer is not a spoof
...
def run_risk_engine(inp: RiskEngineInput) -> RiskResult:
    ...
    # Face detection failure (not a mismatch) — skip hard gate
    ...
    is_aadhaar = inp.document_type in ("national_id", "aadhaar")

    # Never hard-gate Aadhaar checksum: OCR of 12 digits is too noisy.
    if inp.checksum_valid is False:
        if is_aadhaar:
            reasons.append(
                "Aadhaar checksum did not match OCR digits — "
                "treated as inconclusive (OCR noise), not instant FAKE."
            )
        else:
            return RiskResult(
    ...
    # Tiny Aadhaar photos often fail face detection (score 0) — that is not a mismatch.
    if face_detection_failed or (is_aadhaar and inp.face_similarity < FACE_MATCH_HARD_GATE_THRESHOLD):
        reasons.append(
            "Face match inconclusive on this ID photo (small/printed Aadhaar photo) — "
            "not treated as instant FAKE."
        )
    elif inp.face_similarity < FACE_MATCH_HARD_GATE_THRESHOLD:
        return RiskResult(
            band=RiskBand.RED, score=1.0, hard_gated=True,
    ...
    # Single-frame liveness is a proxy, not a blink video — never FAKE a real card on it.
    if inp.blink_score < BLINK_LIVENESS_HARD_GATE_THRESHOLD:
        if is_aadhaar:
            reasons.append(
                f"Liveness score {inp.blink_score:.0f} is low on a still selfie — "
                "not treated as instant FAKE for National ID."
            )
        else:
            return RiskResult(
                band=RiskBand.RED, score=1.0, hard_gated=True,
    ...
    if inp.identity_graph_flagged:
        reasons.append(
            "Identity graph noted a similar face from a prior screening — "
            "logged for review, not instant FAKE (re-testing the same person is common)."
        )
