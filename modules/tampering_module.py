...
class TamperingResult:
    ...
    cnn_texture_score: float = 0.0         # 0-1, LBP texture forgery proxy (heuristic, not trained CNN)
...
# ---------------------------------------------------------------------------
# 3b. LBP texture forgery proxy (CNN slot — heuristic classifier)
# ---------------------------------------------------------------------------

def _score_texture_forgery(image: Image.Image) -> float:
    """
    Local Binary Pattern uniformity score — spliced or AI-generated regions
    often have unnaturally uniform micro-texture vs. a real camera capture.
    Returns 0-1 where higher = more suspicious. Honest label: heuristic proxy,
    not a trained ResNet/EfficientNet.
    """
    ...
    sampled = gray[::step, ::step].astype(np.float32)

    # Simple 3x3 LBP-like pattern: compare center to neighbors
    sh, sw = sampled.shape
    if sh < 3 or sw < 3:
        return 0.0

    center = sampled[1:-1, 1:-1]
    neighbors = [
        sampled[:-2, 1:-1], sampled[2:, 1:-1],
        sampled[1:-1, :-2], sampled[1:-1, 2:],
        sampled[:-2, :-2], sampled[:-2, 2:],
        sampled[2:, :-2], sampled[2:, 2:],
    ]
    patterns = sum((n >= center).astype(np.float32) for n in neighbors) / 8.0
    uniformity = 1.0 - float(np.std(patterns))

    # Very uniform texture → suspicious; natural photos have std ~0.2-0.35
    if uniformity > 0.82:
        return round(min(1.0, (uniformity - 0.82) * 5.0 + 0.3), 4)
    return round(max(0.0, (uniformity - 0.65) * 0.5), 4)
...
def analyze_tampering(
    ...
    # --- Texture forgery proxy (CNN slot) ---
    cnn_texture_score = _score_texture_forgery(image)
    ...
    forgery_probability = round(
        min(
            1.0,
            0.40 * ela_anomaly_score
            + 0.20 * fft_spike_score
            + 0.25 * cnn_texture_score
            + 0.15 * editor_score,
        ),
        4,
    )
    ...
    explanation = _build_explanation(
        ela_anomaly_score, fft_spike_score, editor_score, len(ela_regions), cnn_texture_score
    )
    ...
    return TamperingResult(
        ...
        cnn_texture_score=cnn_texture_score,
...
def _build_explanation(ela_score: float, fft_score: float, editor_score: float, n_regions: int, cnn_score: float = 0.0) -> str:
    ...
    if cnn_score >= 0.20:
        parts.append("texture uniformity suggests possible synthetic or pasted content (LBP heuristic)")
    if editor_score >= 1.0:
...
if __name__ == "__main__":
    ...
    print(f"CNN texture     : {result.cnn_texture_score}")
