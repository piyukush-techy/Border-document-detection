...
DOCUMENT_FIELD_PATTERNS = {
    ...
    "national_id": {  # Aadhaar
        "id_number": r"^\d{4}\s?\d{4}\s?\d{4}$|^\d{12}$",
        "dob": r"^(\d{4}-\d{2}-\d{2}|\d{2}[/-]\d{2}[/-]\d{4})$",
    },
    "driving_license": {
    ...
    "driving_licence": {  # alias for older callers
        "id_number": r"^[A-Z]{2}[0-9]{2}\s?[0-9]{11}$",
        "dob": r"^\d{4}-\d{2}-\d{2}$",
        "expiry_date": r"^\d{4}-\d{2}-\d{2}$",
    },
    "visa": {
    ...
    if doc_type == "national_id":
        ...
        if not id_number:
            return CheckResult(
                ...
                "No Aadhaar number read by OCR — checksum skipped (not treated as fake)",
            )
        if not re.fullmatch(r"\d{12}", id_number):
            return CheckResult(
                "checksum", False, None, None,
                f"Aadhaar number incomplete ({len(id_number)} digits) — checksum skipped",
            )
        is_valid = stdnum_aadhaar.is_valid(id_number)
        return CheckResult(
            "checksum", True, is_valid, 100 if is_valid else 0,
            "Aadhaar Verhoeff checksum " + ("passed" if is_valid else "failed (may be OCR misread)"),
        )
...
def validate_expiry(fields: dict, doc_type: str = "") -> CheckResult:
    """Checks the document hasn't expired, and that dates are logically ordered."""
    # Aadhaar has no expiry date — skip this check entirely
    if doc_type in ("national_id", "aadhaar"):
        return CheckResult("expiry", False, None, None, "Aadhaar has no expiry date — check skipped")
...
class DocumentValidator:
    ...
    def run_all(self, doc_type: str, fields: dict, document_image_path: Optional[str] = None) -> dict:
        checks = [
            ...
            validate_expiry(fields, doc_type),
